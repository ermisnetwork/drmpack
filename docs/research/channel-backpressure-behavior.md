...
1. **Channel Type & Buffer Size:** Bounded `tokio::sync::mpsc::channel(1024)` with a fixed **1024-element** buffer of `PackagedArtifact`, created at [`mod.rs:455`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L455).
2. **When Receiver is NEVER taken (`take_output_receiver()` not called):**
   ...
   - **Cleanup:** On `session.close()`, the session detects `!self.output_receiver_claimed` and sets `self.preserve_output = true` ([`mod.rs:674-676`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L674-L676)). On drop, the output directory is **preserved on disk** for legacy disk-serving mode.
3. **When Receiver IS taken but NEVER polled/consumed:**
   - **RAM & Internal Backpressure:** Channel accepts up to 1024 artifacts then fills. On artifact #1025, Harvester's `tx.send(artifact).await` **suspends indefinitely** ([`harvester.rs:456`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L456)). RAM is **bounded**, not unbounded.
...
4. **Disk Cleanup:**
   - *Receiver dropped:* Harvester detects `tx.send()` error and **exits immediately** ([`harvester.rs:578`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L578)). Files created after that remain on disk. On session drop, the output directory is cleaned up if using a temp directory ([`mod.rs:981-988`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L981-L988)).
...
5. **`PackagingSession` Fate — DEADLOCK RISK:**
   ...
   - On `session.close().await`: **DEADLOCKS INDEFINITELY!** At [`mod.rs:667`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L667), session calls `harvester.finish_and_flush().await`, which awaits the Harvester task's join handle. But the Harvester is blocked on `tx.send(artifact).await` with no cancellation token in the select branch. Session hangs forever at `close()`.
...
- **Location:** [`mod.rs:449-456`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L449-L456)
- **Type:** **Bounded MPSC Channel** (`tokio::sync::mpsc::channel`)
- **Buffer size:** Fixed at **1024 elements** (`PackagedArtifact`)
- **Note:** Inside `Harvester::spawn` ([`harvester.rs:543`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L543)), there is a separate `mpsc::unbounded_channel()`, but that is used internally to bridge filesystem watcher events (`notify::RecommendedWatcher`) into the Tokio runtime — it is not the artifact output channel.
...
- **Lazy Harvester (Zero Overhead if not claimed):**
  - In `PackagingSession::create()` ([`mod.rs:430-431`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L430-L431)): `harvester: None, output_receiver_claimed: false`.
  - Harvester is **only spawned** when caller explicitly calls `session.take_output_receiver()` ([`mod.rs:469-470`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L469-L470)).
...
- **Disk Accumulation:**
  ...
  - Since Harvester is not running, no task unlinks temporary `.m4s` files (normally deleted at [`harvester.rs:448`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L448)).
...
- RAM is **bounded** at ~1024 segments + state history capped at 5000 filenames (`MAX_EMITTED_HISTORY`, [`harvester.rs:12`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L12)).

#### Backpressure Gap with GPAC:
- `PackagingSession::push()` ([`mod.rs:526-601`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L526-L601)) writes directly to GPAC's stdin pipe via `RepresentationCluster::write_data` ([`cluster.rs:199`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/cluster.rs#L199)).
...
**Deadlock Mechanism:**
1. Harvester task is suspended at `tx.send(artifact).await` ([`harvester.rs:456`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L456)) — this call is **NOT** inside a `tokio::select!` with the cancellation token.
2. `close()` calls `harvester.finish_and_flush().await` ([`mod.rs:667`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L667)), which awaits `handle.await` ([`harvester.rs:604`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs#L604)).
...
1. **Fix Deadlock in `harvester.finish_and_flush()`:**
   - Wrap `tx.send(artifact)` with `tokio::select!` against `loop_shutdown.cancelled()` in [`harvester.rs`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/harvester.rs). If cancelled while channel is full, Harvester should break/return instead of hanging.
   - Wrap `tokio::time::timeout()` around `harvester.finish_and_flush()` in [`mod.rs:666-668`](file:///Users/trungdt/Workspace/work/ermis/ermis-stream/drmpack/src/session/mod.rs#L666-L668). On timeout, call `harvester.cancel()` to abort the task.
