...
async fn test_tracer_gpac_e2e_live_packaging() {
    ...
    for _ in 0..50 {
        if init_path.exists() {
            artifact_available_during_push = true;
            break;
        }
        tokio::time::sleep(std::time::Duration::from_millis(20)).await;
    }
    assert!(
        artifact_available_during_push,
        "Output artifacts must be available in Ramdisk during active stream push before close"
    );
    ...
    assert!(
        mpd_content.contains(r#"availabilityTimeComplete="false""#),
        "DASH manifest must signal availabilityTimeComplete=\"false\" for LL-DASH; manifest was:\n{}",
        mpd_content
    );
    assert!(
...
async fn test_tracer_gpac_e2e_cbcs_packaging() {
    ...
    if !has_gpac {
        println!("SKIPPING test_tracer_gpac_e2e_cbcs_packaging: 'gpac' binary not found in PATH");
        return;
    }
    ...
    let config = PackagingSessionConfig::new("e2e-cbcs-live-stream")
        .with_rendition(rendition)
        .with_latency_mode(LatencyMode::LowLatency)
        .with_segment_duration(1.0)
        .with_chunk_duration(0.2)
    ...
    assert!(
        ...
        "CBCS HLS manifest must use METHOD=SAMPLE-AES; manifest was:\n{}",
        m3u8_content
    );
    assert!(
        m3u8_content.contains(r#"KEYFORMAT="com.apple.streamingkeydelivery""#),
        "CBCS HLS manifest must identify Apple FairPlay key format; manifest was:\n{}",
        m3u8_content
    );
    assert!(
    ...
    // Verify DASH manifest contains CBCS protection
    let live_mpd = out_dir.join("live.mpd");
    assert!(live_mpd.exists(), "live.mpd must exist");
    let mpd_content = tokio::fs::read_to_string(&live_mpd).await.unwrap();
    assert!(
        mpd_content.contains(r#"value="cbcs""#) || mpd_content.contains("cbcs"),
        "DASH MPD must signal cbcs protection; manifest was:\n{}",
        mpd_content
    );
    ...
    assert!(
        tenc.windows(16).any(|bytes| bytes == kid.as_bytes()),
        "tenc must carry the ContentKey KID"
    );

    let schm = find_box(&init_bytes, b"schm").expect("Init segment must contain schm box");
    assert!(
        schm.windows(4).any(|b| b == b"cbcs"),
        "schm box must indicate cbcs scheme"
    );
    ...
    for box_type in [b"senc", b"saiz", b"saio"] {
        assert!(
            find_box(&seg_bytes, box_type).is_some(),
            "CBCS media segment must contain {}",
            String::from_utf8_lossy(box_type)
        );
    }
