...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================================
    // PRODUCTION NOTE:
    // In production, your playback authorization API queries PostgreSQL or Redis
    // to retrieve the `DrmStreamMetadata` for this stream_id:
    //
    //   let meta_json = db.get_stream_drm(stream_id).await?;
    //   let metadata = DrmStreamMetadata::from_json(&meta_json)?;
    //
    // For this offline demo, we read `drm_metadata.json` generated by example 08
    // to simulate querying that database record automatically, avoiding the need
    // to manually type or copy-paste KIDs between terminals.
    // =========================================================================
    ...
    println!(
        "Loaded DRM Stream Metadata (mock DB) for content: {}",
        metadata.content_id
    );
